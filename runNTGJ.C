             ...
             const char *run_mode = "test")
{
        ...
        if (strcmp(key, "package") == 0) {
            if (strncmp(value, "GCC::", 5) != 0) {
                ...
                if (gSystem->AccessPathName(include.ReplaceAll(
                    ...
                    // TString::ReplaceAll already modified the
                    // content)
                    include = "$ALICE_ROOT/../../" +
            ...
            package_override.Add((TObject *)(new TString(value)));
        }
    }
    fclose(fp);

    const char *package_default[] = {
    ...
    if (package_override.GetSize() == 0) {
        for (const char **p = package_default; *p != NULL; p++) {
    ...
    for (Int_t i = 0; i < package_override.GetSize(); i++) {
        plugin->AddExternalPackage(
            *((TString *)package_override.At(i)));
    }
